...
dag_name_dtls = {'dag_name': "goldCore_ga4_wom_kpi_raw_objects_daily_bld"}

object_properties_json = "/mapr/JMAPRCLUP01.CLASSIC.PCHAD.COM/codebase/bigdata/prod/gcp/gold_core/config/bigdata/variables/group_binding_id_104/json_122.json"
...
for i in object_details.object_id_list_sorted:
    ...
    recon_start_dt = date.today() - timedelta(int(object_details.recon_start_limit))

    if object_details.recon_start_limit == 0:
        recon_end_dt = date.today()
    else:
        recon_end_dt = date.today() - timedelta(int(object_details.recon_end_limit))
