...
def report(recon_date:int, property_id:str, credentials_path:str, dimensions: List[str], metrics: List[Metric], row_limit:int=10000 ,quota_usage:bool=False):
    ...
    en_date = str(date.today() - timedelta(days=recon_date))
           ...
           RunReportRequest(
                ...
                dimension_filter=FilterExpression(
                        and_group=FilterExpressionList(
                            expressions=[
                                FilterExpression(filter=Filter(field_name="hostName",in_list_filter= Filter.InListFilter(values=hostName, case_sensitive=False),)),
                            ]
                        )
                    ), 
...
result = report(
    ...
    recon_date=8,
    dimensions=["hostName", "country", "date"], # ga:hostname, ga:country, ga:channelGrouping
    metrics=["activeUsers", "eventCount", "eventValue"], # ga:users, ga:pageviews, ga:sessions
    row_limit=100000
)
