    ...
    CASE
        ...
        WHEN (drv.hdl_loadtype = 'INCREMENTAL' AND ref1.appendmode = 'Y' AND drv.hdl_subtype <> 'RELOAD')
            THEN CASE
                ...
                when (ref4.jobstatus_for_appnd = 'Failed') Then COALESCE(ref4.upperboundvalue, 0) + (SELECT lastboundvalue+1 
                    FROM (
    ...
    CASE
        ...
        WHEN (drv.hdl_loadtype = 'INCREMENTAL' AND ref1.appendmode = 'Y' AND drv.hdl_subtype <> 'RELOAD')
            THEN CASE
                ...
                when (ref4.jobstatus_for_appnd = 'Failed') Then (COALESCE(ref4.upperboundvalue, 0) + (SELECT lastboundvalue+ref1.threshold_for_upperlmt 
                    FROM (
                        SELECT  lastboundvalue, mapping_id,jobendtime
                        FROM gcp_ingestion_lastrun
                        GROUP BY mapping_id 
...
) tab where mapping_id in (72,73,123)