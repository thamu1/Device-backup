...
with dag:
    ingest_source_task = PythonOperator(
        task_id='ingest_source',
        python_callable=ingestion_source('/path/to/ingestion_source.json', subjectarea_id=1).insert,
    )

    ingest_controller_task = PythonOperator(
        task_id='ingest_controller',
        python_callable=ingestion_controller('/path/to/ingestion_controller.json').insert,
    )

    ingestion_task = BashOperator(
        task_id='run_ingestion',
        bash_command='spark-submit ingestion_script.py',
    )

    ingest_source_task >> ingest_controller_task >> ingestion_task
